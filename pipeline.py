...
LUA_SHA1 = get_hash(os.path.join(CWD, 'xfire.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "xfire-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("xfire-user: %(item_name)s"),
        ]
...
project = Project(
    title="xfire",
    project_html="""
        <img class="project-logo" alt="Project logo" src="http://archiveteam.org/images/thumb/0/0f/Logo_Xfire.png/320px-Logo_Xfire.png" height="50px" title=""/>
        <h2>social.xfire.com <span class="links"><a href="http://social.xfire.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/xfire/">Leaderboard</a></span></h2>
        <p>Saving data from Xfire.</p>
    """,
    utc_deadline=datetime.datetime(2015, 6, 12, 23, 59, 0)
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="xfire"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
